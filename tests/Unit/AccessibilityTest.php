...
test(
	'returns correct text color for background color', function () {
	$a11y = app( A11y::class );
...
test(
	'correctly checks the contrast between two colors', function () {
	$a11y = app( A11y::class );
...
test(
	...
	$a11y = app( A11y::class );
	// Malformed hex triggers fallback to black
...
test(
	'handles malformed hex codes in a11yCheckContrastColor', function ( $malformedHex ) {
	$a11y = app( A11y::class );
